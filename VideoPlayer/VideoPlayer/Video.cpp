...
bool Video::Start()
{
	std::string file = "videos/AnimationVideo.wmv";
	...
	scaled_frame->format = AVPixelFormat::AV_PIX_FMT_YUV420P; // Format used for SDL
	scaled_frame->width = dst_w; //width of window
	scaled_frame->height = dst_h; //height of window
	av_frame_get_buffer(scaled_frame, 0); //get buffers for YUV format
...
bool Video::Update(float dt)
{
	...
	{
		frame_amount++;
		while (frame_amount >= frame_ratio)
		{
